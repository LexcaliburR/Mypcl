...
__global__ void calc_conv_indices_stage1_mask(
    ConvLocIter loc_iter, const int* indices_in, int32_t* indice_pairs_bwd,
    int32_t* indice_pairs_for_uniq, int* indice_num_per_loc, int num_indices_in,
    int RS, bool transposed)
{
    ...
    for (int input_index : tv::KernelLoopX<int>(num_indices_in)) {
        ...
        bool valid;
        if (transposed) {
            valid = loc_iter.query_nhw_out(indices_in + input_index * 4,
                                           npq_offset);
        } else {
            valid =
                loc_iter.query_npq(indices_in + input_index * 4, npq_offset);
        }
        if (valid) {
            int old_num =
                tv::cuda::atomicAggInc(indice_num_per_loc + filter_offset);
            int32_t output_coord_offset = loc_iter.layout_npq(npq_offset);
            // if (old_num < indices_pair_size){
            // indice_pairs[filter_offset_mul_indices_pair_size + old_num] = i;
            indice_pairs_bwd[filter_offset_mul_indices_pair_size +
                             input_index] = output_coord_offset;
            indice_pairs_for_uniq[filter_offset_mul_indices_pair_size +
                                  old_num] = output_coord_offset;
            // }
        }
    }
}
