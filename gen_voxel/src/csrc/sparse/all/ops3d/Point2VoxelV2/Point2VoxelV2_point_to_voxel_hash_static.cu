...
Point2VoxelV2::point_to_voxel_hash_static(
    ...
    std::array<float, 6> coors_range, bool clear_voxels, bool padding,
    std::uintptr_t stream_int)
{
    ...
    // int64_t expected_hash_data_num =
    // int64_t(tv::hash::align_to_power2(points.dim(0) * 2));
    int64_t expected_hash_data_num = points.dim(0) * 2;
    TV_ASSERT_RT_ERR(hashdata.dim(0) >= expected_hash_data_num,
                     "hash table too small")
    TV_ASSERT_RT_ERR(point_indice_data.dim(0) >= points.dim(0),
                     "point_indice_data too small")
    ...
    launcher(kernel::build_hash_table<table_t>,
             hash,
             points.data_ptr<const float>(),
    ...
    table_launcher(kernel::assign_table<table_t>,
                   hash,
    ...
    launcher(kernel::generate_voxel<table_t>,
             hash,
             points.data_ptr<const float>(),
             point_indice_data.data_ptr<const int64_t>(),
             voxels.data_ptr<float>(),
             num_per_voxel.data_ptr<int>(),
             points_voxel_id.data_ptr<int64_t>(),
             points.dim(1),
             voxels.dim(1),
             voxels.dim(0),
             vsize_tv,
    ...
    voxel_launcher(kernel::voxelout_gather_mean,
                   voxels.data_ptr<float>(),
                   voxels_out.data_ptr<float>(),
                   num_per_voxel.data_ptr<int>(),
                   count_val,
                   voxels.dim(1),
                   voxels.dim(2));

    voxel_launcher(kernel::indicesout_gather,
                   indices.data_ptr<int>(),
    ...
    voxel_launcher(kernel::limit_num_per_voxel_value,
                   num_per_voxel.data_ptr<int>(),
                   count_val,
                   voxels.dim(1));

    if (padding) {
        return std::make_tuple(voxels_out, indices_out, num_per_voxel);
    } else {
        return std::make_tuple(voxels_out.slice_first_axis(0, count_val),
                               indices_out.slice_first_axis(0, count_val),
