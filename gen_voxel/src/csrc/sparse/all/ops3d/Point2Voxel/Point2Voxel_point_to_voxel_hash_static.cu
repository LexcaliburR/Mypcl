...
Point2Voxel::point_to_voxel_hash_static(
    tv::Tensor points, tv::Tensor voxels, tv::Tensor indices,
    tv::Tensor num_per_voxel, tv::Tensor hashdata, tv::Tensor point_indice_data,
    tv::Tensor points_voxel_id, std::array<float, 3> vsize,
    std::array<int, 3> grid_size, std::array<int, 3> grid_stride,
    std::array<float, 6> coors_range, bool clear_voxels, bool empty_mean,
    std::uintptr_t stream_int)
{
    ...
    if (empty_mean) {
        launcher(kernel::voxel_empty_fill_mean,
                 voxels.data_ptr<float>(),
                 num_per_voxel.data_ptr<int>(),
                 count_val,
                 voxels.dim(1),
        ...
        launcher(kernel::limit_num_per_voxel_value,
                 num_per_voxel.data_ptr<int>(),
                 count_val,
                 voxels.dim(1));
    }
