...
struct TensorGeneric
{
    ...
    TV_HOST_DEVICE_INLINE TensorGeneric(tv::array<int, 3> const& strides)
        : strides(strides)
    {}
    TV_HOST_DEVICE_INLINE static TensorGeneric from_shape(
        const tv::array<int, 4>& shape)
    {
        return TensorGeneric(
            {shape[1] * shape[2] * shape[3], shape[2] * shape[3], shape[3]});
    }
    TV_HOST_DEVICE_INLINE int64_t
    operator()(const tv::array<int, 4>& indexes) const
    {
        return indexes[3] + int64_t(strides[0] * indexes[0]) +
               int64_t(strides[1] * indexes[1]) +
               int64_t(strides[2] * indexes[2]);
    }
