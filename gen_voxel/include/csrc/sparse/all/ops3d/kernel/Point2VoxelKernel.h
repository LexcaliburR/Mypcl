...
__global__ void build_hash_table(TTable table, float const* points,
                                 int64_t* points_indice_data, int point_stride,
                                 tv::array<float, 3> vsize,
                                 tv::array<float, 6> coors_range,
                                 tv::array<int, 3> grid_bound,
                                 tv::array<int, 3> grid_stride, int num_points)
{
...
#pragma unroll
        for (int j = 0; j < 3; ++j) {
            c = floor((points[i * point_stride + 2 - j] - coors_range[j]) /
                      vsize[j]);
...
template <typename TTable>
__global__ void assign_table(TTable table, int* indices, int* count,
                             Layout layout, int max_voxels)
{
    ...
    for (int i : tv::KernelLoopX<int>(table.size())) {
        ...
        if (!item.empty()) {
            ...
            if (item.second < max_voxels) {
                layout.inverse(item.first, indices + item.second * 3);
            }
        }
    }
}
template <typename TTable>
__global__ void generate_voxel(TTable table, float const* points,
                               const int64_t* points_indice_data, float* voxels,
                               int* num_per_voxel, int64_t* points_voxel_id,
                               int point_stride, int max_points_per_voxel,
                               int max_voxels, tv::array<float, 3> vsize,
                               tv::array<float, 6> coors_range,
                               tv::array<int, 3> grid_bound,
                               tv::array<int, 3> grid_stride, int num_points)
{
            ...
            if (!voxel_index_pair.empty() &&
                ...
                if (old < max_points_per_voxel) {
                    for (int j = 0; j < point_stride; ++j) {
                        voxels[voxel_index_pair.second * voxel_stride0 +
                               old * point_stride + j] =
 ...
 */
__global__ void voxel_empty_fill_mean(float* voxels, int* num_per_voxel,
